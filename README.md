...
> - **LoKR training** (v3.0) — the highest likeness we've ever measured here, with noticeably more natural skin than standard LoRA. Network Type dropdown on the Krea 2 Training tab; one Factor dial (keep it at 8); Repair Studio and Explorer save LoKR natively, lossless. [Training ↓](#training)
> - **One-click cloud training on RunPod** — nothing to install, and the in-app panel auto-stops the pod when your run finishes so an idle GPU never bills overnight. [**⚡ Deploy →**](https://console.runpod.io/deploy?type=GPU&gpu=RTX+5090&count=1&template=faoq8ed6um&ref=vkb387ep) · [Guide](docker/README.md)
...
> - **Never lose a run** (v2.11) — state saves at every checkpoint and at run end; a finished LoRA can be trained further by raising the epoch count and resuming.
> - **Caption with the model that trains your LoRA** (v2.10) — Qwen3-VL captioning, every task an editable preset, including a style-LoRA preset validated on real training runs.
